...
export default class SessionI {
	...
	async restoreSession() {
		var session = null;		return this.getSessionFromStorage().then((token) => {
			if (token && token != '') {
				// Session.restore requires both auth token and refresh token, 
				// but we only have the auth token stored. Pass null for refresh token.
				session = Session.restore(token, null);
