		...
		connect.socket.onmatchdata = (md) => {
			...
			switch (op_code) {
				...
				case 3:
					...
					break;
				case 4:
					game.playerManager.activeCreature.moveTo(game.grid.hexes[md.data.target.y][md.data.target.x]);
					game.gamelog.add({
						action: 'move',
						target: {
							x: md.data.target.x,
					...
					break;
				case 5:
					const args = Array.from(md.data.args[1]);

					if (md.data.target.type == 'hex') {
						args.unshift(game.grid.hexes[md.data.target.y][md.data.target.x]);
						game.playerManager.activeCreature.abilities[md.data.id].animation2({
							arg: args,
						});
						game.gamelog.add({
							action: 'ability',
					...
					if (md.data.target.type == 'creature') {
						args.unshift(game.creatureManager.creatures[md.data.target.crea]);
						game.playerManager.activeCreature.abilities[md.data.id].animation2({
							arg: args,
						});
						game.gamelog.add({
