...
export class Drop {
	...
	constructor(name: string, alterations: DropAlterations, x: number, y: number, game: Game) {
		...
		this.game.playerManager.drops.push(this);

		const px = this.hex.displayPos;
		this.display = game.grid.dropGroup.scene.add.sprite(
			px.x + 54,
			px.y + 15,
			'drop_' + this.name
		);
		...
		game.grid.dropGroup.add(this.display);
	...
	pickup(creature: Creature) {
		...
		game.gameManager.log('%CreatureName' + creature.id + '% picks up ' + this.name + ':');
		...
		game.gameManager.log(`Gains ${gainedMessage}`);
	...
	destroy() {
		...
		this.game.playerManager.drops = this.game.playerManager.drops.filter((drop) => drop !== this);
	}
