...
export class Ability {
	...
	constructor(creature: Creature, abilityID: AbilitySlot, game: Game) {
		...
		this.timesUsedThisTurn = 0;
		this.token = 0;
		this.upgraded = false;		this.title = '';
		const data = game.creatureManager.retrieveCreatureStats(creature.type);
		
		// Add null checks to handle cases where abilities haven't loaded yet
		const abilityData = game.playerManager.abilities[data.id] && game.playerManager.abilities[data.id][abilityID];
		const abilityInfo = data.ability_info && data.ability_info[abilityID];
		
		if (abilityData) {
			Object.assign(this, abilityData);
		}
		if (abilityInfo) {
			Object.assign(this, abilityInfo);
		}
	...
	hasUpgrade() {
		return this.game.gameManager.abilityUpgrades >= 0 && this.upgraded;
	}
	...
	usesLeftBeforeUpgrade(): number {
		...
		if (this.isUpgradedPerUse()) {
			return game.gameManager.abilityUpgrades - this.timesUsed;
		}

		return game.gameManager.abilityUpgrades - this.creature.turnsActive;
	}
	...
	use() {
		...
		if (this.used === true) {
			game.gameManager.log('Ability already used!');
			return;
		}

		game.gameManager.clearOncePerDamageChain();
		game.playerManager.activeCreature.hint(this.title, 'msg_effects');
	...
	//TODO: Once all files in `abilities` are converted to TS, consider defaulting both of these arguments to `false`.
		...
		if (!disableLogMsg) {
			game.gameManager.log('%CreatureName' + this.creature.id + '% uses ' + this.title);
		}
	 ...
	 */	setUsed(val: boolean): void {
		...
		if (val) {
			...
			if (this.creature.id == game.playerManager.activeCreature.id) {
				if (game.UI && game.UI.abilitiesButtons && game.UI.abilitiesButtons[this.id]) {
	...
	postActivate(): void {
		...
		if (this.isUpgraded() && this.usesLeftBeforeUpgrade() == 0) {
			game.gameManager.log(this.title + ' has been upgraded');
	 ...
	 */	animation2(o: object): void {
		...
		if (this.getTrigger() === 'onQuery') {
			...
			let animationData = {
				...
				// Create a tween chain using Phaser 3 syntax
				const tween = game.createTween(this.creature.sprite, { x: p1 }, 250, 'Linear');
				tween.to({ x: p2 }, 100, 'Linear');
				tween.to({ x: p0 }, 150, 'Linear');
			}
	...
	atLeastOneTarget(hexes: Hex[], o): boolean {
		...
		this.message = this.game.msg.playerManager?.abilities?.noTarget || this.game.msg.abilities?.noTarget || 'No target available';
		return false;
	}
	...
	testRequirements(): boolean {
		...
		req = { ...def, ...this.requirements },
		abilityMsgs = game.msg.playerManager?.abilities || game.msg.abilities;
		// Plasma
		const plasmaReq = typeof req.plasma === 'string' ? parseInt(req.plasma, 10) : req.plasma;
		if (plasmaReq > 0) {
			if (this.creature.player.plasma < plasmaReq) {
	...
	applyCost() {
		...
		Object.entries(this.costs).forEach(([key, value]: [string, number]) => {
			if (typeof value == 'number') {
				if (key == 'health') {
					creature.hint(value.toString(), 'damage');
					game.gameManager.log('%CreatureName' + creature.id + '% loses ' + value + ' health');
				} else if (key === 'energy') {
					value += creature.stats.reqEnergy;
				}
		...
		if (creature.id == game.playerManager.activeCreature.id) {
	...
	testDirection(o: object): boolean {
		...
		this.message = this.game.msg.playerManager.abilities.noTarget;
		return false;
	}
