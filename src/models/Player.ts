...
export class Player {
	 ...
	 */	summon(type: CreatureType, pos: Point) {
		...
		const baseCreatureData = game.creatureManager.retrieveCreatureStats(type);
	...
	flee(o) {
		this.hasFled = true;
		this.deactivate();
		this.game.gameManager.skipTurn(o);
	}
	...
	isLeader(): boolean {
		...
		for (let i = 0; i < game.playerMode; i++) {
			...
			if (game.playerManager.players[i].getScore().total > this.getScore().total) {
	...
	deactivate(): void {
		...
		if (game.playerMode > 2) {
			// 2 vs 2
			if (game.playerManager.players[(this.id + 2) % 4].hasLost) {
