...
if (audioPlayer) {
	...
	if (timeline) {
		timeline.addEventListener(
			'click',
			(e: MouseEvent) => {
				// Use offsetWidth for numeric width
				const timelineWidth = (timeline as HTMLElement).offsetWidth;
				const timeToSeek = (e.offsetX / timelineWidth) * skin.duration;
...
setInterval(() => {
	if (audioPlayer) {
		const progressBar = audioPlayer.querySelector('.progress-music') as HTMLElement | null;
		if (progressBar) {
			progressBar.style.width = (skin.currentTime / skin.duration) * 100 + '%';
		}
		const currentTime = audioPlayer.querySelector('.time .current') as HTMLElement | null;
		if (currentTime) {
...
function getTimeCodeFromNum(num: number): string {
