	...
	G.playerManager.abilities[0] = [
		// 	First Ability: Plasma Field
		{
			...
			activate: function (damage) {
				if (G.playerManager.activeCreature.id == this.creature.id) {
				...
				if (this.isUpgraded() && damage.melee && !damage.counter) {
					...
					counter.counter = true;					G.playerManager.activeCreature.takeDamage(counter);
					G.cameraShake(0.03, 220, true, 'HORIZONTAL', true);
				}
				...
				G.gameManager.log('%CreatureName' + this.creature.id + '% is protected by Plasma Field');
				return damage; // Return Damage
			},
		},
		...
		// 	Second Ability: Electro Shocker
		{
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.02, 200, true, 'HORIZONTAL', true);

				const damageAmount = {
					shock: 12 * target.size,
				};				const damageObj = new Damage(
		...
		// 	Third Ability: Disruptor Beam
		{
			...
			require: function () {
				...
				if (this.creature.player.plasma < lowestCost) {
					this.message = G.msG.playerManager.abilities.noPlasma;
					return false;
				}
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.04, 111, true, 'HORIZONTAL', true);

				const plasmaCost = target.size;
				let damage = target.baseStats.health - target.health;				if (this.isUpgraded() && damage < 40) {
					damage = 40;
				}
		...
		// 	Fourth Ability: Godlet Printer
		{
			...
			require: function () {
				...
				if (this.creature.player.getNbrOfCreatures() == G.creaLimitNbr) {
					this.message = G.msG.playerManager.abilities.noPsy;
					return false;
				}
				return true;
			},
			...
			materialize: function (creature) {
				...
				const creatureHasMaterializationSickness =
					dpriest.player.summonCreaturesWithMaterializationSickness;				// Create full temporary Creature with placeholder position to show in queue
				crea = Object.assign(
					crea,
					{ x: 3, y: 3 },
				...
				fullCrea.sprite.alpha = 0;				// Show temporary Creature in queue
				G.gameManager.updateQueueDisplay();
