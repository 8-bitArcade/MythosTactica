	...
	G.playerManager.abilities[44] = [
		...
		// 	Second Ability: Slicing Pounce
		{
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.01, 70, true, 'HORIZONTAL', true);
				...
				if (this.isUpgraded()) {
					...
					G.gameManager.log('%CreatureName' + target.id + "%'s offense is lowered by 1");
				}
		...
		// 	Third Ability: Escort Service
		{
			...
			//	activate() :
			activate: function (hex, args) {				const ability = this;
				ability.end();
				G.cameraShake(0.01, 66, true, 'HORIZONTAL', true);
		...
		// 	Fourth Ability: Deadly Toxin
		{
			...
			activate: function (target) {				const ability = this;
				...
				const damages = Object.assign({}, ability.damages);
				if (!this.isUpgraded()) {
				...
				if (result.damageObj.status !== 'Shielded') {
					...
					const effect = new Effect(
						...
						'onStartPhase',
						{						stackable: false,
						effectFn: function (eff, target) {
							// Type assertion to ensure target is a Creature
							const creature = target as Creature;
							// Type assertion to ensure eff.owner is a Creature
							const effectOwner = eff?.owner as Creature;
							
							G.gameManager.log('%CreatureName' + creature.id + '% is affected by ' + ability.title);
							creature.takeDamage(
								new Damage(
									effectOwner,
									{
					...
					G.gameManager.log('%CreatureName' + target.id + '% is poisoned by ' + this.title);
				}
			},
		},
	];
