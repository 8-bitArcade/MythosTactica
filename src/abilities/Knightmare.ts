	...
	G.playerManager.abilities[9] = [
		...
		// 	Second Ability: Icy Talons
		{
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.01, 80, true, 'HORIZONTAL', true);
		...
		// 	Third Ability: Sudden Uppercut
		{
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.02, 222, true, 'VERTICAL', true);
			...
			activate: function (path) {
				...
				for (let i = 0; i < path.length; i++) {
					if (path[i].creature instanceof Creature) {
						...
						G.cameraShake(0.02, 80, true, 'HORIZONTAL', true);
