	...
	G.playerManager.abilities[5] = [
		...
		// 	Second Ability: Hasted Javelin
		{
			...
			activate: function (target) {				const ability = this;
				ability.end();
				G.cameraShake(0.01, 120, true, 'HORIZONTAL', true);

				const finalDmg = Object.assign(
					{
						poison: 0,
					},
					ability.damages1,
				...
				if (result.damages && result.damages.total > 0) {
					...
					G.gameManager.log('%CreatureName' + this.creature.id + "%'s movement recharged");
			...
			activate: function (target) {
				...
				target.addEffect(
					new Effect(
						...
						target,
						'onUnderAttack',						{
							effectFn: (effect, damageArg) => {
								if (damageArg instanceof Damage) {
									const dmg = damageArg.applyDamage();
				...
				const effect = new Effect(
					...
					'onStepOut',
					{
						effectFn: once((effect) => {
							G.gameManager.log('%CreatureName' + target.id + '% is injured by ' + effect.name);
		...
		//	Fourth Ability: Chain Lightning
		{
			...
			activate: function (target) {
				const ability = this;
				ability.end();				const targets = [];
				targets.push(target); // Add First creature hit
				let nextdmg = Object.assign({}, ability.damages); // Copy the object
					...
					if (this.isUpgraded() && isTeam(this.creature, trg, Team.Ally)) {
						trg.addEffect(
							new Effect(
								...
								trg,
								'onUnderAttack',								{
									effectFn: function (effect, damageArg) {
										if (damageArg instanceof Damage) {
											const damage = damageArg;
