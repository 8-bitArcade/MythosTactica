	...
	G.playerManager.abilities[7] = [
		 ...
		 */
		{
			...
			activate(path, args) {
				...
				const damage = this._getDamage(path);				tween.onComplete.add(function (this: any) {
		...
		// Third Ability: Bonfire Spring
		{
			...
			activate(hex) {
				...
				const effectFn = function (this: any, effect, creatureOrHex) {
				...
				const requireFn = function (this: any) {
				...
				G.grid.queryHexes({
					...
					fnOnSelect: function (hex) {
						range.forEach(function (item) {
							...
							item.overlayVisualState('creature selected player' + G.playerManager.activeCreature.team);
						});
						hex.cleanOverlayVisualState();
						hex.overlayVisualState('creature selected player' + G.playerManager.activeCreature.team);
					},
					id: this.creature.id,
					hexes: range,
					hideNonTarget: true,
